...
public class ExperimentManager : MonoBehaviour
{
    ...
    public GameObject locomotionTarget;
    ...
    private void ResetTargets()
    {
        ...
        {
            // place the target at its spawn center
            t.transform.position = t.spawnCenter;

            // try to disable/enable appropriate GameObjects depending on hierarchy
            GameObject targetGO = t.gameObject;
            GameObject rootGO = t.transform.root != null ? t.transform.root.gameObject : null;

            if (tutorialMode)
            {
                // disable target visuals: prefer disabling the target's own GameObject, then root
                if (targetGO != null)
                    targetGO.SetActive(false);
                if (rootGO != null && rootGO != targetGO)
                    rootGO.SetActive(false);
            }
            else
            {
                // enable both to restore normal behavior
                if (rootGO != null)
    ...
    public void SetTutorialMode(bool status)
    {
        ...
        if (tutorialMode)
        {
            Debug.Log("[ExperimentManager] Tutorial Mode enabled.");
            //TODO: disable targets here
            locomotionTarget.SetActive(false);
        ...
        {
            Debug.Log("[ExperimentManager] Tutorial Mode disabled.");
            //TODO: enable targets here
            locomotionTarget.SetActive(true);
            PlacePlayerAtWorldSpawn();
    ...
    public void SetFadeEnabled(bool enabled)
    {
        ...
        // Push the flag to all BallisticTeleport instances in the scene
        var players = FindObjectsOfType<BallisticTeleport>();
